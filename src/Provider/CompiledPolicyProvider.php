 ...
 *
 * In tolerant mode, malformed or stale descriptors return null so
 * CompositePolicyProvider can continue to the AttributePolicyProvider fallback.
 * In strict mode, such descriptors throw InvalidCompiledPolicyException.
 */
final class CompiledPolicyProvider implements PolicyProviderInterface
{
    ...
    public function provideFor(string $actionId): ?PolicyInterface
    {
        ...
        $policy = $this->build($actionId, $descriptor);
    ...
    private function build(string $actionId, array $descriptor): ?PolicyInterface
    {
        ...
        try {
            return match ($kind) {
                ...
                'all_of' => $this->buildComposite($actionId, $descriptor, all: true),
                'one_of' => $this->buildComposite($actionId, $descriptor, all: false),
                default => null,
            };
        } catch (InvalidCompiledPolicyException $exception) {
            throw $exception;
        } catch (\Throwable $exception) {
            if ($this->strict) {
                throw InvalidCompiledPolicyException::forAction($actionId, $exception);
            }
