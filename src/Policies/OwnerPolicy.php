...
final class OwnerPolicy extends AbstractPolicy
{
    ...
    public function enforce(object $actor, ContextInterface $context): true|DenyReason
    {
        ...
        $resource = $context->requireAttribute(self::ATTR_RESOURCE);
        ...
        $ownerUuid = $resource->getOwnerUuid();

        if ($ownerUuid instanceof UuidInterface) {
            return $ownerUuid->equals($actor->uuid)
                ? true
                : $this->deny('Actor is not the owner of this resource');
        }

        if ($ownerUuid === $actor->uuid->toString()) {
            return true;
        }

        return $this->deny('Actor is not the owner of this resource');
    }
}
