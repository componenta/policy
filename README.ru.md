...
- если политика не найдена и действует `MissingPolicyBehavior::DENY`, метод возвращает `AccessDeniedInterface`;
- если политика вернула `true`, метод возвращает `true`;
- если политика вернула `DenyReason`, метод оборачивает его в `AccessDeniedInterface` с `actionId`, актором и контекстом;
...
`PermissionPolicy` умеет читать разрешения напрямую из `PermissionAwareInterface` и из роли через `RoleAwareInterface`. Если у актора есть оба источника, они объединяются через `CompositePermissionCollection`: наличие разрешения в любом источнике достаточно.
...
`PermissionCollectionInterface` — контракт только для чтения: `contains()`, `toArray()`, итерация и `count()`. Конкретный `PermissionCollection` дополнительно дает `add()` и `remove()` для инфраструктуры, сидеров и фикстур.
...
`ContextInterface` — иммутабельное хранилище пар ключ-значение для данных конкретной проверки. Массив, переданный в `PolicyEnforcer`, автоматически преобразуется через `ContextFactoryInterface`.
...
`requireAttribute()` выбрасывает `MissingPolicyContextAttributeException`, если значения нет, и `InvalidPolicyContextAttributeException`, если значение есть, но тип не подходит.

## Встроенные политики

| Политика | Что проверяет | Что требуется |
|---|---|---|
| `PermissionPolicy` | У актора есть все или любое из указанных разрешений. | `PermissionAwareInterface` и/или `RoleAwareInterface`. |
| `RolePolicy` | Имя роли актора входит в список разрешенных ролей. | `RoleAwareInterface` или `RoleInterface`. |
| `HierarchyPolicy` | Роль актора выше роли целевого пользователя. | Актор с ролью и `target` в контексте. |
| `OwnerPolicy` | Актор владеет ресурсом. | Актор `IdentityInterface`, ресурс `OwnableInterface` в контексте под ключом `resource`. |
| `AlwaysPolicy` | Фиксированное разрешение или отказ. | Ничего. |
...
`CompiledPolicyProvider` читает дескрипторы политик, сгенерированные `componenta/policy-app`. Он нужен для боевого окружения, где не хочется искать атрибуты через рефлексию на горячем пути. Если дескриптор устарел или невалиден, провайдер возвращает `null`, и следующий провайдер в цепочке может обработать действие.
...
|---|---|
| Политика вернула `DenyReason` | `check()` возвращает `AccessDeniedInterface`, `enforce()` выбрасывает `AccessDeniedException`. |
