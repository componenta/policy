...
`ActorProviderInterface` получает субъекта из конкретного источника интеграции, например HTTP-запроса, сессии или токена аутентификации. Возвращаемые значения имеют разные значения:
