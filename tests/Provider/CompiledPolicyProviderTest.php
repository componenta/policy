...
function compiledPolicyProviderForTests(array $classes, bool $strict = false): CompiledPolicyProvider
{
...
describe('Provider\CompiledPolicyProvider', function () {
    ...
    it('throws for malformed descriptors in strict mode', function () {
        $provider = new CompiledPolicyProvider(new FakeFactory(), [
            'broken' => ['kind' => 'direct', 'class' => 'MissingPolicy', 'arguments' => []],
        ], strict: true);
    ...
    it('still returns null for actions missing from the compiled map in strict mode', function () {
        $provider = new CompiledPolicyProvider(new FakeFactory(), [], strict: true);

        expect($provider->provideFor('unknown.action'))->toBeNull();
    });
});
